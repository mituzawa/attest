...
#ifdef CONFIG_TPM
static void create_fdt_tpm(RISCVVirtState *s, const MemMapEntry *memmap)
{
    char *nodename;
    MachineState *mc = MACHINE(s);
    hwaddr base = memmap[VIRT_TPM].base;
    hwaddr size = memmap[VIRT_TPM].size;

    nodename = g_strdup_printf("/soc/tpm@%" PRIx64, base);
    qemu_fdt_add_subnode(mc->fdt, nodename);
    qemu_fdt_setprop_string(mc->fdt, nodename,
...
#ifdef CONFIG_TPM
static void create_tpm_tis(const MemMapEntry *memmap)
{
    DeviceState *dev;
    SysBusDevice *sbd;
    hwaddr base = memmap[VIRT_TPM].base;
    ...
    qdev_prop_set_string(dev, "tpmdev", "tpm0");
